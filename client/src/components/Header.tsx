  ...
  return (
    <AppBar position="static">
      <Toolbar>
        ...
        <Box sx={{ minWidth: 120 }}>
          <Typography variant="body1">Sitzplatz: {currentUser ? currentUser.seat : ''}</Typography>
          <Typography variant="body1">Admin: {currentUser ? (currentUser.isGlobalAdmin ? 'Ja' : 'Nein') : ''}</Typography>
        </Box>
        <Box sx={{ minWidth: 120 }}>
          ...
          <FormControl variant="outlined" size="small" sx={{ backgroundColor: 'white', borderRadius: 1 }}>
            {loading ? (
              ...
              <Typography variant="body2" sx={{ p: 1, color: 'red' }}>
                Fehler
              </Typography>
            ) : (
              <Select value={currentUser ? String(currentUser.id) : ''} onChange={handleChange} displayEmpty sx={{ minWidth: 150 }}>
                <MenuItem value="">
                  <em>Wähle User</em>
                </MenuItem>
                {data?.users.map((user) => (
